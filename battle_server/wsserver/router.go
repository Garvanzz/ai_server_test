...
//
//// OnConnect 链接进来
//func (r *LockStepServer) OnConnect(conn *wsnetwork.Conn) bool {
...
//
//// OnMessage 消息处理
//func (r *LockStepServer) OnMessage(conn *wsnetwork.Conn, p pb_packet.IPacket) bool {
//	msg := p.(*pb_packet.Packet)
//
//	protoMsg, err := proto_id.NewMessage(msg.GetMessageID())
//	if err != nil {
//		log.Error("LockStepServer Onmessage parse proto error:%v", err)
//		return false
//	}
//
...
//		_, err := r.mgr.CreateGame(rec.GetRoomInfo(), rec.GetLogicServer(), conn, nil)
//		if err != nil {
...
//		log.Info("[router] creategame Addr=[%v]: %v", conn.GetRawConn().RemoteAddr(), re)
//		return true
...
//		return room.OnConnect(conn, nil)
...
//
//// OnClose 链接断开
//func (r *LockStepServer) OnClose(conn *wsnetwork.Conn) {
//	count := atomic.AddInt64(&r.totalConn, -1)
//
