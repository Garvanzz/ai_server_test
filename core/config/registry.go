...
// registry 存储所有配置表的解析函数，在 tables.go 的包级变量初始化阶段填充。
var registry = map[string]func(raw any, name string) any{}

// ---------------------------------------------------------------------------
// Table — 数组类配置表 (JSON 数组 → map[int64]T)
// ---------------------------------------------------------------------------

// Table 提供对 map[int64]T 配置表的类型安全访问。
type Table[T any] struct {
	name string
}

// NewTable 注册一张配置表并返回其类型安全访问器。
// 包级变量初始化时调用，数据在 InitConfig 完成后可用。
...
func (t *Table[T]) Get(id int64) T {
	m := CfgMgr.AllJson[t.name].(map[int64]T)
	v, ok := m[id]
	if !ok {
...
func (t *Table[T]) All() map[int64]T {
	return CfgMgr.AllJson[t.name].(map[int64]T)
}
...
func (s *Single[T]) Get() T {
	return CfgMgr.AllJson[s.name].(T)
}
