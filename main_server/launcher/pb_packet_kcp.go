...
func NewPacket(id uint32, msg interface{}) *Packet {
	...
	case proto.Message:
		if mdata, err := proto.Marshal(v); err == nil {
			...
			log.Error("[NewPacket] proto marshal msg: %d error: %v",
				id, err)
			return nil
		}
	case nil:
	default:
		log.Error("[NewPacket] error msg type msg: %d", id)
		return nil
	}
...
func (p *MsgProtocol) ReadPacket(r io.Reader) (*Packet, error) /*Packet*/ {
...
func (p *MsgProtocol) ReadPacketByByte(bufs []byte) (*Packet, error) /*Packet*/ {
