...
func (mgr *Manager) OnCreateGuild(ctx *proto_player.Context, req *proto_guild.C2SCreateGuild) bool {
	...
	// 检查是否有重名的帮会
	rdb, _ := db.GetEngine()
	exist, err := rdb.Mysql.Table(define.GuildTable).Where("server_id = ? AND guild_name = ?", mgr.App.GetEnv().ID, req.Name).Exist()
	if err != nil {
...
func (mgr *Manager) OnLeaveGuild(ctx *proto_player.Context, req *proto_guild.C2SLeaveGuild) (resp *proto_guild.S2CLeaveGuild) {
	...
	if ent.getProp(define.GuildPropMaster) == int32(info.Id) {
		...
		n, err := rdb.Mysql.Table(define.GuildTable).Where("id = ? AND server_id = ?", ent.guild.Id, mgr.App.GetEnv().ID).Delete()
		if err != nil {
...
func (mgr *Manager) OnGetEvents(ctx *proto_player.Context, req *proto_guild.C2SGuildEvent) (resp *proto_guild.S2CGuildEvent) {
	...
	err := rdb.Mysql.Table(define.GuildLogTable).Where("server_id = ? AND guild_id = ?", mgr.App.GetEnv().ID, info.GuildId).Desc("create_time").Limit(define.LogCountMax).Find(&logs)
	if err != nil {
