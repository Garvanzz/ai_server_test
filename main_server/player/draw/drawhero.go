...
func Save(pl *model.Player, isSync bool) {
	...
	if isSync {
		...
		if err != nil {
			log.Error("save draw error, no this server:%v", err)
			return
		}
		rdb.RedisExec("SET", fmt.Sprintf("%s:%d", define.PlayerDraw, pl.Id), j)
	} else {
		// TODO: 异步存储
...
func Load(pl *model.Player) {
	...
	reply, err := rdb.RedisExec("GET", fmt.Sprintf("%s:%d", define.PlayerDraw, pl.Id))
	if err != nil {
