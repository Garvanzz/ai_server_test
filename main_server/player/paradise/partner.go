...
func ReqInitPartner(ctx global.IPlayer, pl *model.Player, req *proto_huaguoshan.C2SInitPartner) {
	...
	// 初始化数据
	if pl.Paradise == nil || pl.Paradise.Partner == nil {
		initParadiseData(pl)
	}

	// 检查每日重置
	checkDailyReset(pl.Paradise.Partner)

	// 获取伴侣信息
	var partnerInfo *proto_public.CommonPlayerInfo
	if pl.Paradise.Partner.PartnerId > 0 {
		partnerInfo = getPlayerInfo(ctx, pl.Paradise.Partner.PartnerId)
	}
	log.Debug("pl.Paradise.Partner: %v", pl.Paradise.Partner)
	resp.Option = pl.Paradise.Partner.ToPartnerOption(partnerInfo)
	ctx.Send(resp)
}
...
func ReqGetPartnerInviteList(ctx global.IPlayer, pl *model.Player, req *proto_huaguoshan.C2SGetPartnerInviteList) {
	...
	// 从logic层获取邀请列表
	invites := invoke.ParadiseClient(ctx).GetReceiverInvites(pl.Id)
...
func ReqPartnerInvite(ctx global.IPlayer, pl *model.Player, req *proto_huaguoshan.C2SPartnerInvite) {
	...
	// 校验: 自己不在冷却期
	if isInCooldown(pl.Paradise.Partner) {
	...
	invite := invoke.ParadiseClient(ctx).CreateInvite(pl.Id, pl.Base.Name, req.Id)
	if invite == nil {
...
func ReqLogicPartnerInvite(ctx global.IPlayer, pl *model.Player, req *proto_huaguoshan.C2SLogicPartnerInvite) {
	...
	// 处理邀请
	invite, err := invoke.ParadiseClient(ctx).ProcessInvite(req.Id, req.Status)
	if err != nil {
	...
	// 同意邀请: 建立伴侣关系
	// 设置自己的伴侣信息
	pl.Paradise.Partner.PartnerId = invite.SenderId
	pl.Paradise.Partner.Intimacy = 0
	pl.Paradise.Partner.IntimacyLevel = 0
	pl.Paradise.Partner.PartnerType = 0
	pl.Paradise.Partner.LastRelieveTime = 0
	pl.Paradise.Partner.GiveCount = 0
	pl.Paradise.Partner.LastGiveResetTime = utils.Now().Unix()
	pl.Paradise.Partner.UnlockedSkills = []int32{}
	pl.Paradise.Partner.UnlockedBraces = []int32{}
	pl.Paradise.Partner.UnlockedMounts = []int32{}
	pl.Paradise.Partner.UnlockedHeadWears = []int32{}
	pl.Paradise.Partner.UnlockedBuffs = []int32{}
	pl.Paradise.Partner.UnlockedHeadFrames = []int32{}
...
func ReqRelievePartner(ctx global.IPlayer, pl *model.Player, req *proto_huaguoshan.C2SRelieveParterner) {
	...
	partnerId := pl.Paradise.Partner.PartnerId

	// 清空伴侣数据
	pl.Paradise.Partner.PartnerId = 0
	pl.Paradise.Partner.Intimacy = 0
	pl.Paradise.Partner.IntimacyLevel = 0
	pl.Paradise.Partner.PartnerType = 0
	pl.Paradise.Partner.LastRelieveTime = utils.Now().Unix()
	pl.Paradise.Partner.GiveCount = 0
	pl.Paradise.Partner.UnlockedSkills = []int32{}
	pl.Paradise.Partner.UnlockedBraces = []int32{}
	pl.Paradise.Partner.UnlockedMounts = []int32{}
...
func ReqPartnerGive(ctx global.IPlayer, pl *model.Player, req *proto_huaguoshan.C2SParternerGive) {
	...
	if pl.Paradise.Partner.GiveCount >= day {
	...
	// 增加自己的亲密度
	pl.Paradise.Partner.Intimacy += addIntimacy

	// 检查是否升级
	checkIntimacyLevelUp(pl)

	// 增加赠送次数
	pl.Paradise.Partner.GiveCount++
	...
	if len(cost) > 0 {
		...
		invoke.MailClient(ctx).SendMail(
			...
			"", "", "系统",
			costItem,
			[]int64{pl.Paradise.Partner.PartnerId},
			0, 0, false, []string{},
		)
	}

	resp.GiveCount = pl.Paradise.Partner.GiveCount
	resp.Intimacy = pl.Paradise.Partner.Intimacy
	resp.IntimacyLevel = pl.Paradise.Partner.IntimacyLevel

	log.Debug("Player %d give gift %d to partner %d, intimacy: %d", pl.Id, req.Id, pl.Paradise.Partner.PartnerId, pl.Paradise.Partner.Intimacy)
	ctx.Send(resp)
}

// initParadiseData 初始化乐园数据
func initParadiseData(pl *model.Player) {
	if pl.Paradise == nil {
		pl.Paradise = &model.Paradise{}
	}
	if pl.Paradise.Partner == nil {
		pl.Paradise.Partner = &model.ParadisePartner{
			PartnerId:          0,
			Intimacy:           0,
			IntimacyLevel:      0,
			PartnerType:        0,
	...
	if pl.Paradise.Wine == nil {
		pl.Paradise.Wine = &model.ParadiseWine{
			CurMakingWineId:       0,
	...
	if pl.Paradise.Peach == nil {
		pl.Paradise.Peach = &model.ParadisePeach{
			CurTreeId:              0,
...
// isInCooldown 是否在冷却期
func isInCooldown(partner *model.ParadisePartner) bool {
...
func checkIntimacyLevelUp(pl *model.Player) {
	...
	for _, cfg := range intimacyConfigs {
		if pl.Paradise.Partner.Intimacy >= cfg.Exp {
	...
	// 如果等级有变化，解锁对应奖励
	if newLevel > pl.Paradise.Partner.IntimacyLevel {
		oldLevel := pl.Paradise.Partner.IntimacyLevel
		pl.Paradise.Partner.IntimacyLevel = newLevel
...
func unlockRewards(pl *model.Player, level int32, configs map[int64]conf.ParternerIntimacy) {
	...
	if cfg.UnLockBuffValue > 0 {
		pl.Paradise.Partner.UnlockedBuffs = append(pl.Paradise.Partner.UnlockedBuffs, cfg.UnLockBuffValue)
	}

	// 解锁头像框
	if cfg.UnLockHeadFrameValue > 0 {
		pl.Paradise.Partner.UnlockedHeadFrames = append(pl.Paradise.Partner.UnlockedHeadFrames, cfg.UnLockHeadFrameValue)
	}

	// 解锁头饰
	if cfg.UnLockHeadWearValue > 0 {
		pl.Paradise.Partner.UnlockedHeadWears = append(pl.Paradise.Partner.UnlockedHeadWears, cfg.UnLockHeadWearValue)
	}

	// 解锁背饰
	if cfg.UnLockBraceValue > 0 {
		pl.Paradise.Partner.UnlockedBraces = append(pl.Paradise.Partner.UnlockedBraces, cfg.UnLockBraceValue)
	}

	// 解锁坐骑
	if cfg.UnLockMountValue > 0 {
		pl.Paradise.Partner.UnlockedMounts = append(pl.Paradise.Partner.UnlockedMounts, cfg.UnLockMountValue)
	}

	// 解锁技能
	if cfg.UnLockSkillValue > 0 {
		pl.Paradise.Partner.UnlockedSkills = append(pl.Paradise.Partner.UnlockedSkills, cfg.UnLockSkillValue)
	}
}
